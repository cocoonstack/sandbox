...
multi-arch manifests (`browser`, `desktop` and `android` remain amd64-only). Build from source with
...
| `archive_after_seconds` | 0 (off) | tier below hibernation: a hibernated claim idle this long is checkpointed to the store and its local VM dropped, freeing the node entirely; the next call that reaches the guest restores it transparently (a checkpoint restore's latency) with a fresh server-default 5m lease — whatever TTL the claim first asked for, and nothing renews it. Keep `idle_hibernate_seconds` under those 5 minutes: a woken sandbox that idles can then hibernate and be archived again, where at 300 or more the lease runs out first and the reap destroys it, its archive already consumed by the wake. A sandbox still in use when the lease ends is destroyed the same way. Requires `idle_hibernate_seconds > 0` and must exceed it. Node-wide for unpooled keys; per-pool overrides for that pool |
