...
| `checkpoint_dir` | `<data_dir>/checkpoints` | where checkpoints and promoted templates live. Point it at a shared FUSE mount (JuiceFS over object storage, NFS) and every node sharing the mount can branch every checkpoint — the path's filesystem is the operator's choice, and the storage backend sits behind an interface for future native object-store support. One caveat on any shared root (mount or bucket): record writes are serialized per node, not across the cluster — two nodes promoting the same template name concurrently is last-writer-wins (checkpoint ids are node-generated and never collide). Cross-node conditional publish is tracked in issue #22 |
