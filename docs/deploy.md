...
| `refill_concurrency` | 0 (auto) | concurrent VM provisioning budget, shared by warm-pool refills, fork clones, and the reap/hibernate/reconcile engine batches. 0 sizes it from the node: `NumCPU/16` clamped to [4, 64] — a 384-core node gets 24 (~200 fills/s at ~120 ms per golden clone), small nodes keep the historical 4 |
