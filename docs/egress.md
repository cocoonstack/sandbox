...
still needs no resolver. Each door admits at most 256 open connections per
sandbox; a dial past that waits in the socket backlog until one closes, so a
guest cannot turn its proxy into a host descriptor sink.
