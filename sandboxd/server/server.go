...
type Manager interface {
	ClaimWarm(ctx context.Context, key types.PoolKey, ttl time.Duration, tenant, claimRef string, ws types.WorkspaceSpec, volumes []types.Volume) (*types.Sandbox, error)
	ClaimProvision(ctx context.Context, key types.PoolKey, ttl time.Duration, tenant, claimRef string, ws types.WorkspaceSpec, volumes []types.Volume) (*types.Sandbox, error)
	ClaimProvisionPromoted(ctx context.Context, key types.PoolKey, ttl time.Duration, tenant, claimRef string, ws types.WorkspaceSpec, volumes []types.Volume) (*types.Sandbox, error)
...
func (s *Server) handleClaim(w http.ResponseWriter, r *http.Request) {
	...
	sb, err := s.mgr.ClaimWarm(r.Context(), key, req.TTL(), tenant, req.ClaimRef, req.WorkspaceSpec(), nil)
	if errors.Is(err, pool.ErrNoWarm) {
		if s.redirectClaim(r.Context(), w, req, key, hash) {
			return
		}
		sb, err = s.mgr.ClaimProvision(r.Context(), key, req.TTL(), tenant, req.ClaimRef, req.WorkspaceSpec(), nil)
	}
...
func (s *Server) handleVolumeClaim(w http.ResponseWriter, r *http.Request, req types.ClaimRequest, key types.PoolKey, hash, tenant string) {
	...
	if req.RequirePromoted {
		sb, err = s.mgr.ClaimProvisionPromoted(r.Context(), key, req.TTL(), tenant, req.ClaimRef, req.WorkspaceSpec(), req.Volumes)
	} else {
		sb, err = s.mgr.ClaimWarm(r.Context(), key, req.TTL(), tenant, req.ClaimRef, req.WorkspaceSpec(), req.Volumes)
		if errors.Is(err, pool.ErrNoWarm) {
			if s.placer != nil && !req.NoRedirect && writeRedirect(w, s.placer.VolumeCandidates(hash, types.VolumeNames(req.Volumes))) {
				return
			}
			sb, err = s.mgr.ClaimProvision(r.Context(), key, req.TTL(), tenant, req.ClaimRef, req.WorkspaceSpec(), req.Volumes)
		}
	}
	writeResult(w, r, "claim", hash, "provisioning failed", err, func() {
		writeJSON(w, http.StatusOK, s.claimResponse(sb))
