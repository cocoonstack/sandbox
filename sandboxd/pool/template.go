...
func (m *Manager) resolveGolden(ctx context.Context, key types.PoolKey) (string, string, bool, func(), error) {
	m.mu.Lock()
	var dir string
	...
	if key.Net == types.NetEgress {
		return "", "", false, func() {}, nil // never resume a live-captured template on the egress lane; cold-boot instead
	}
	id := store.TemplateID(key.Hash())
	l := m.recLock(id)
	l.RLock()
	...
	if err := json.Unmarshal(meta, &rec); err != nil {
		...
		l.RUnlock()
		m.recDone(id)
		return "", "", false, func() {}, fmt.Errorf("decode template metadata: %w", err)
	}
	return dir, digest, true, func() { release(); l.RUnlock(); m.recDone(id) }, nil
}
