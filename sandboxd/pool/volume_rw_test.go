...
func TestWritableClaimMarksDirtyBeforeAttachAndClearsOnRelease(t *testing.T) {
	...
	sb, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "scratch", Mode: types.VolumeModeRW}})
	if err != nil {
...
func TestClaimRejectsWriteOnReadOnlyEntry(t *testing.T) {
	...
	_, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "data", Mode: types.VolumeModeRW}})
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "scratch"}}); err != nil {
...
func TestDirtyVolumeBlocksReadersUntilWriterRecovers(t *testing.T) {
	...
	_, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, readOnly)
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, readOnly); err != nil {
...
func TestDirtyVolumeRefusesWarmClaimBeforeTakingAVM(t *testing.T) {
	...
	_, err := m.ClaimWarm(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "scratch"}})
	if !errors.Is(err, ErrVolumeNeedsRecovery) {
...
func TestVolumeAdmissionMatrix(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			first, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "scratch", Mode: tt.first}})
			if err != nil {
				t.Fatalf("first claim: %v", err)
			}
			before := eng.volumeOpsLog()
			_, err = m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, second)
			if tt.wantErr == nil {
				if err != nil {
			...
			if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, second); err != nil {
...
func TestVolumeAdmissionRefusalHoldsNothing(t *testing.T) {
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "held", Mode: types.VolumeModeRW}}); err != nil {
		t.Fatalf("first claim: %v", err)
	}

	_, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "free", Mode: types.VolumeModeRW}}); err != nil {
...
func TestVolumeAdmissionReleasedAfterSetupFailure(t *testing.T) {
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, writable); err == nil {
	...
	if _, err := m.ClaimWarm(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, writable); !errors.Is(err, ErrNoWarm) {
...
func TestReapQuiescesWritableVolumesBeforeRemoval(t *testing.T) {
	...
	eng := newFakeEngine()
	m := newVolumeManager(t, eng, []config.VolumeSpec{{Name: "scratch", Path: path, Writable: true}})
	sb, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "scratch", Mode: types.VolumeModeRW}})
	if err != nil {
		t.Fatalf("ClaimProvision: %v", err)
	}
	m.mu.Lock()
...
func TestQuiesceFailureSyncsOnceAndKeepsMarkers(t *testing.T) {
	...
	sb, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "first"}}); !errors.Is(err, ErrVolumeNeedsRecovery) {
...
func TestPendingRemovalHoldsVolumesUntilDrained(t *testing.T) {
	...
	m := newVolumeManager(t, eng, []config.VolumeSpec{{Name: "scratch", Path: path, Writable: true}})
	writable := []types.Volume{{Name: "scratch", Mode: types.VolumeModeRW}}
	sb, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, writable)
	if err != nil {
		t.Fatalf("ClaimProvision: %v", err)
	}
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, writable); !errors.Is(err, ErrVolumeBusy) {
...
func TestReconcileRebuildsVolumeAdmission(t *testing.T) {
	...
	writable := []types.Volume{{Name: "scratch", Mode: types.VolumeModeRW}}
	if _, err := m2.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, writable); !errors.Is(err, ErrVolumeBusy) {
	...
	if _, err := m2.ClaimProvision(t.Context(), testKey, 0, "", "", types.WorkspaceSpec{}, []types.Volume{{Name: "scratch"}}); err != nil {
