...
func TestTenantQuotaBindsPerTenant(t *testing.T) {
	...
	first, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "acme", "", types.WorkspaceSpec{}, nil)
	if err != nil {
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "acme", "", types.WorkspaceSpec{}, nil); !errors.Is(err, ErrQuota) {
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "beta", "", types.WorkspaceSpec{}, nil); err != nil {
		t.Errorf("beta claim while acme is at cap: %v", err)
	}
	if _, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "", "", types.WorkspaceSpec{}, nil); err != nil {
	...
	if _, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "acme", "", types.WorkspaceSpec{}, nil); err != nil {
...
func TestTenantStampedInJournals(t *testing.T) {
	eng := newFakeEngine()
	m := newTestManager(t, eng)
	sb, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "acme", "", types.WorkspaceSpec{}, nil)
	if err != nil {
