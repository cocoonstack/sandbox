...
func TestPromoteFailsClosedOnMetaError(t *testing.T) {
	...
	eng := newFakeEngine()
	m := newTestManager(t, eng)
	a, err := m.ClaimProvision(t.Context(), testKey, time.Hour, "acme", "", types.WorkspaceSpec{}, nil)
	if err != nil {
		t.Fatalf("claim: %v", err)
	}
...
func TestPromoteRefusesCrossTenantOverwrite(t *testing.T) {
	...
	claim := func(tenant string) *types.Sandbox {
		t.Helper()
		sb, err := m.ClaimProvision(t.Context(), testKey, time.Hour, tenant, "", types.WorkspaceSpec{}, nil)
		if err != nil {
