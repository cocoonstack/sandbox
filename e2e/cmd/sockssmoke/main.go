...
// refused, IMAPS rides the tunnel, and a pool without a policy has no
// listener at all.
package main
...
func run(addr, token, template, echo, getOnly, imap string) error {
	...
	if code := curl(ctx, bare, "--socks5-hostname", socksProxy, "http://"+echo+"/get"); code != "000" {
		return fmt.Errorf("policy-less pool served SOCKS5 with %q, want a refused connection", code)
	}
	fmt.Println("  pool without a policy has no SOCKS5 listener")
	return nil
}
